...
class EraCV:
    ...
    def get_valid_indices(self, valid_start, valid_n_eras):
        self.valid_eras = self.unique_eras[
            self.unique_eras.index(valid_start) : self.unique_eras.index(valid_start)
            + valid_n_eras
        ]
    ...
    def get_train_indices(
        self, valid_start: int, train_start: int = 0, train_stop: int = None
    ):
        train_stop = valid_start if (train_stop is None) else train_stop
        self.train_eras = [era for era in self.unique_eras if era < valid_start][
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(" f"last era:{max(self.eras_int)})"
